...
export const createServiceMap = function (constructor) {
  ...
  if (!serviceMap[serviceName]) {
    serviceMap[serviceName] = new (class
      extends constructor
      implements IAxiosfit
    {
      ...
      setInterceptors(
        ...
        for (const interceptor of interceptors) {
          if (
            (interceptor as AxiosfitRequestInterceptor).onRequest !== undefined
          ) {
            this.axiosInstance.interceptors.request.use(
              (interceptor as AxiosfitRequestInterceptor).onRequest,
              (interceptor as AxiosfitRequestInterceptor).onError ||
                defaultError
            );
          }
          if (
            ...
            this.axiosInstance.interceptors.response.use(
              (interceptor as AxiosfitResponseInterceptor).onResponse,
              (interceptor as AxiosfitResponseInterceptor).onError ||
                defaultError
            );
          }
        }
        if (this.axiosfitConfig?.enableAxiosLogger) {
          this.axiosInstance.interceptors.request.use(
            axiosfitLogger.onRequest,
            axiosfitLogger.onError
          );
          this.axiosInstance.interceptors.response.use(
            axiosfitLogger.onResponse,
            axiosfitLogger.onError
          );
        }
      }
      ...
      addUrl(key: string, url: string): void {
        this.urlMap[
          key
        ] = `${this.axiosRequestConfig.baseURL}${this.baseServiceEndpoint}${url}`;
      }
