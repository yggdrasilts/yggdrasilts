...
describe('StringUtils', () => {
  ...
  describe('replaceAll', () => {
    ...
    it('should replace all "key" for "egg"', () => {
      expect(StringUtils.replaceAll('The key is the key.', 'key', 'egg')).toBe('The egg is the egg.');
    });
  });
  describe('replace', () => {
    it('should replace "a" for "b"', () => {
      expect(StringUtils.replace('This is a replacement test.', 'a', 'b')).toBe('This is b replacement test.');
    });
    it('should replace "key" for "egg"', () => {
      expect(StringUtils.replace('The key is the key.', 'key', 'egg')).toBe('The egg is the key.');
    });
  });
});
